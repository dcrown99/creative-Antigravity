...
```bash
uv run python download_images_as_cbz.py "URL" --output-dir "C:\保存先のパス"
```
...
```bash
uv run python download_images_as_cbz.py "URL" --name "マイ漫画_第1話"
```
...
# ファイル名を指定
uv run python download_images_as_cbz.py "URL" --name "ファイル名"

# 保存先を指定
uv run python download_images_as_cbz.py "URL" --output-dir "C:\保存先"

# ファイルから一括ダウンロード
uv run python download_images_as_cbz.py --file urls.txt

# 一時フォルダを残す（デバッグ用）
uv run python download_images_as_cbz.py "URL" --keep-temp

# 複数のURLを一度に指定
uv run python download_images_as_cbz.py "URL1" "URL2" "URL3"

# ヘルプを表示
uv run python download_images_as_cbz.py --help
```
